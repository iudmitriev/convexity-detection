...
if __name__ == '__main__':
    ...
    assert convex_detector.is_convex('x * ln(x) + 1', symbol_space={'x': interval.Interval([0, float('inf')])})

    X = sym.MatrixSymbol('X', 3, 1)
    func = X.T * X + 2 * sym.Identity(1)
    assert convex_detector._is_convex_expression(func)
    ...
    assert convex_detector.is_convex(expr, matrix_symbol_dict=matrix_symbol_dict)
    print('Finished')
